...
{
	++ps->chunk;
	while ((ab && ps->st->end && ps->i > 0 && (ps->st->count > 2)
		&& *(ps->st->end->nb) < m) ||
...
//		ft_ps_check_zero(ps);
	...
	while (ps->st->start && ps->i > 0 && ((ab && ps->st->count > 2 && (*(ps->st->start->nb) < m) && ps->st->start->chunk) || (!ab && *(ps->st->start->nb) > m)))
	{
		ft_check_duble(ps, ps->st, ps->st->end, 0);
